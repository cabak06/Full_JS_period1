...
describe("Create/Update Comments", () => {
  ...
  it("Should find the user Donald Duck", async () => {
    ...
    const user = await fetch(`${URL}/api/users/dd@b.dk`, config).then(r => r.json());
  ...
  it("Should not find the user xxx@b.dk", async () => {
   ...
   const user = await fetch(`${URL}/api/users/xxx@b.dk`, config).then(r => r.json());
  }catch (err){
  ...
  it("Should Remove the user Donald Duck", async () => {
    ...
    expect(UserFacade.users.length).to.be.equal(2);
